...
export default function InternshipsPage() {
  ...
  const handleAddToTracker = async (internship: Internship) => {
    try {
      setAddingId(internship._id);
      await trackerAPI.add({
        internshipId: internship._id,
        status: 'apply-later',
      });
      // Show success message (you can add toast notification here)
      alert(`Added ${internship.company} to your tracker!`);
    } catch (err: any) {
      alert(err.response?.data?.message || 'Failed to add to tracker');
    } finally {
      setAddingId(null);
    }
  };
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    disabled={addingId === internship._id}
